...
static power_state_fn_t power_state_preon(const Tus::PowerState                    &p,
...
static power_state_fn_t power_state_on(const Tus::PowerState &p, const Ux::System::PowerStateChangeReason &reason)
{
    switch (p)
    {
        case Tus::PowerState::Off:
        {
            ...
            // Tell the RPi task to prepare for power off
            Teufel::Task::RpiLink::postMessage(ot_id, set_power_msg);
            SyncPrimitive::await(Tus::Task::RpiLink, 4000, "Powering off RPi");
...
static power_state_fn_t power_state_off(const Tus::PowerState &p, const Ux::System::PowerStateChangeReason &reason)
{
    switch (p)
    {
        case Tus::PowerState::On:
        {
            ...
            p_power_state.set(Tus::PowerState::PreOn, getDesc(Tus::PowerState::PreOn));

            return reinterpret_cast<power_state_fn_t>(power_state_preon);
        }
        default:
            break;
    }
...
static power_state_fn_t power_state_preon(const Tus::PowerState &p, const Ux::System::PowerStateChangeReason &reason)
{
    switch (p)
    {
        case Tus::PowerState::On:
        {
            log_highlight("Off->PreOn");

            auto set_power_msg = p_power_state.setTransition(Tus::PowerState::On, reason, getDesc(Tus::PowerState::On));
